export function initKeyManagement() {
    ...
    const modalHTML = `
        <div class="key-modal-overlay" id="key-management-modal" style="
            ...
            <div class="key-modal-content" style="
                ...
                border-radius: 24px; padding: 20px; width: 92%; max-width: 380px;
                max-height: 92vh; overflow-y: auto; color: #f8fafc;
                ...
                <div style="margin-bottom: 12px;">
                    <div style="font-size: 10.5px; font-weight: 600; color: #38bdf8; margin-bottom: 5px; letter-spacing: 0.5px;">5v5 MODE KEYS</div>
                ...
                <!-- 1v1 Mode Keys -->
                <div style="margin-bottom: 12px;">
                    <div style="font-size: 10.5px; font-weight: 600; color: #38bdf8; margin-bottom: 5px; letter-spacing: 0.5px;">1v1 MODE KEYS</div>
                    <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 5px;" id="grid-1v1">
                        ${renderKeys('1v1', ['5k', '10k', '15k', '25k', '50k'], keyData)}
                    </div>
                </div>

                <!-- Tournament Key (Compact Size) -->
                <div style="margin-bottom: 14px;">
                    <div style="font-size: 10.5px; font-weight: 600; color: #c084fc; margin-bottom: 5px; letter-spacing: 0.5px;">TOURNAMENT KEY</div>
                    <div style="background: rgba(30, 41, 59, 0.5); border: 1px solid rgba(192, 132, 252, 0.3); border-radius: 10px; padding: 8px 12px; display: flex; justify-content: space-between; align-items: center;">
                        <span style="font-size: 10px; color: #94a3b8;">Tournament Pass</span>
                        <span style="font-size: 13px; font-weight: bold; color: #c084fc;">${keyData.modes.tournament.pass} pcs</span>
                    </div>
                </div>

                <!-- Refund System Box with KPay Inputs & Custom Dropdown Styling -->
                <div style="background: rgba(30, 41, 59, 0.5); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 16px; padding: 12px; margin-bottom: 14px;">
                    ...
                    <div style="display: flex; gap: 6px; align-items: center; margin-bottom: 8px;">
                        <select id="refund-key-select" style="
                            ...
                            border-radius: 10px; color: #f8fafc; padding: 7px 8px; font-size: 10px; outline: none; cursor: pointer;
                            appearance: none; -webkit-appearance: none; background-image: url('data:image/svg+xml;utf8,<svg fill=\"%2338bdf8\" height=\"20\" viewBox=\"0 0 24 24\" width=\"20\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M7 10l5 5 5-5z\"/></svg>'); background-repeat: no-repeat; background-position: right 6px center;">
                        ...
                        <select id="refund-qty-select" style="
                            ...
                            border-radius: 10px; color: #f8fafc; padding: 7px 8px; font-size: 10px; outline: none; cursor: pointer;
                            appearance: none; -webkit-appearance: none; background-image: url('data:image/svg+xml;utf8,<svg fill=\"%2338bdf8\" height=\"20\" viewBox=\"0 0 24 24\" width=\"20\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M7 10l5 5 5-5z\"/></svg>'); background-repeat: no-repeat; background-position: right 4px center;">
                    ...
                    <div style="display: flex; gap: 6px; margin-bottom: 8px;">
                        <input type="text" id="kpay-name" placeholder="KPay Account Name" style="
                            ...
                            border-radius: 10px; color: #f8fafc; padding: 7px 10px; font-size: 10px; outline: none;">
                        <input type="text" id="kpay-phone" placeholder="KPay Phone No." style="
                            flex: 1; background: rgba(15, 23, 42, 0.95); border: 1px solid #334155;
                            border-radius: 10px; color: #f8fafc; padding: 7px 10px; font-size: 10px; outline: none;">
                    </div>

                    <!-- Clear and prominent info text -->
                    <div id="refund-info-text" style="font-size: 11px; color: #38bdf8; font-weight: 600; line-height: 1.3; min-height: 28px; margin-bottom: 8px; background: rgba(56, 189, 248, 0.1); border-left: 2px solid #38bdf8; padding: 6px 8px; border-radius: 0 8px 8px 0;">
                    ...
                    <button id="execute-refund-btn" style="
                        ...
                        border-radius: 10px; color: white; padding: 8px; font-size: 11px;
    ...
    function updateInfoText() {
        ...
        refundInfoText.innerText = `Exchange ${qty} pcs of ${mode.toUpperCase()} ${type.toUpperCase()} keys for ${totalVal} Ks.`;
    }
        ...
        return types.map(type => {
            ...
            return `
                <div style="
                    ...
                    border-radius: 8px; padding: 6px 3px; text-align: center; box-sizing: border-box;">
                    <div style="font-size: 8.5px; color: #94a3b8; margin-bottom: 2px;">${type.toUpperCase()}</div>
                    <div style="font-size: 12px; font-weight: bold; color: ${count > 0 ? '#38bdf8' : '#64748b'};">
                        ${count} <span style="font-size: 7.5px; font-weight: normal;">pcs</span>
                    </div>
                </div>
            `;
        }).join('');
    ...
    executeRefundBtn.addEventListener('click', () => {
        ...
        if (!selectedVal || !qty) {
            alert('Please select both key type and quantity for refund.');
            return;
        }

        if (!kpayName || !kpayPhone) {
            alert('Please enter your KPay Account Name and Phone Number.');
            return;
        }
        ...
        const isConfirmed = confirm(`Refund ${qty} pcs of ${mode.toUpperCase()} ${type.toUpperCase()} keys for ${totalVal.toLocaleString()} Ks to KPay (${kpayName} - ${kpayPhone})?`);
        ...
        if (keyData.modes[mode] && keyData.modes[mode][type] >= qty) {
            ...
            updateUI(keyData);
            alert(`Successfully requested refund for ${qty} pcs of ${mode.toUpperCase()} (${type.toUpperCase()}) keys!`);
        } else {
